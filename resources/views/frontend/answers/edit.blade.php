                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.answers.update", [$answer->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="reply_id" id="reply_id" required>
                                @foreach($replies as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('reply_id') ? old('reply_id') : $answer->reply->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('reply'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="answer" id="answer" value="{{ old('answer', $answer->answer) }}" required>
